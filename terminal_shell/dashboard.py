...
class ConsoleDashboard(QMainWindow):
    ...
    def _compose_ui(self):
        ...
        target_layout.addWidget(self._make_field_label("URL or slug"), 0, 0, 1, 2)
        ...
        target_layout.addWidget(self._make_field_label("Folder"), 2, 0, 1, 2)
        ...
        target_layout.addLayout(folder_line, 3, 0, 1, 2)

        target_layout.addWidget(self._make_field_label("Mode"), 4, 0)
        self.specialization_toggle = QCheckBox("Specialization")
        target_layout.addWidget(self.specialization_toggle, 4, 1)

        target_layout.addWidget(self._make_field_label("Video"), 5, 0)
        ...
        target_layout.addWidget(self.quality_select, 5, 1)

        target_layout.addWidget(self._make_field_label("Subtitles"), 6, 0)
        ...
        target_layout.addWidget(self.subtitle_select, 6, 1)
        ...
        browser_line = general.get_supported_browsers_text()
        browser_note = QLabel(f"Sign in first. Supported: {browser_line}.")
        browser_note.setObjectName("SectionHint")
        browser_note.setWordWrap(True)
        options_layout.addWidget(browser_note)
    ...
    def _compose_command(self, resume=False):
        ...
        self.preference_store.update('argdict.classname', self.target_input.text().strip())
        ...
        runtime_options = {}
        specialization_mode = False
        for key, value in self.preference_store.get_full_db()['argdict'].items():
            if key == 'classname':
                raw_target = self.preference_store.read('argdict')['classname']
                default_kind = 'specialization' if self.specialization_toggle.isChecked() else 'course'
                parsed_target = general.parse_coursera_target(raw_target, default_kind=default_kind)
                if parsed_target is None:
                    self._show_message_box(
                        "Error",
                        "Invalid course, specialization, or professional certificate URL/slug.",
                        QMessageBox.Warning,
                    )
                    return None

                runtime_options[key] = parsed_target.slug
                specialization_mode = parsed_target.kind == 'specialization' or self.specialization_toggle.isChecked()
                continue
    ...
    def _finish_transfer(self, success, error_type, message):
        ...
        if error_type == 'ConnectionError':
            detail = "Failed to connect to Coursera. Check your internet connection and try again."
        elif error_type == 'HTTPError':
            detail = "Coursera returned an HTTP error. Make sure you are logged in and enrolled in the target course."
