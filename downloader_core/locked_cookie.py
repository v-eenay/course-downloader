...
DEFAULT_COOKIES_PATH = os.path.expandvars(r"%LOCALAPPDATA%\Google\Chrome\User Data\Default\Network\Cookies")
...
def unlock_cookies(cookie_file=DEFAULT_COOKIES_PATH):
    ...
    try:
        result = DWORD(rstrtmgr.RmRegisterResources(session_handle, 1, byref(pointer(create_unicode_buffer(cookie_file))), 0, None, 0, None)).value
...
def fetch_locked_cookies(domain):
    unlock_cookies(DEFAULT_COOKIES_PATH)
    # return browser_cookie3.chrome(cookies_path)
    return browser_cookie3.chrome(domain_name=domain)


@backoff.on_exception(backoff.constant, PermissionError, max_tries=5)
def fetch_locked_chromium_cookies(domain, cookie_file, loader, key_file=None):
    unlock_cookies(cookie_file)
    return loader(cookie_file=cookie_file, domain_name=domain, key_file=key_file)
