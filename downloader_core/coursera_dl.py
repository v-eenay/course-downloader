...
from general import loadcauth_result
...
def create_session(args):
    ...
    if args.cookies_cauth:
        ...
        cauth_cookie, cookie_error, source_browser = loadcauth_result('coursera.org', args.browser)
        if not cauth_cookie:
            raise RuntimeError(
                f'Could not load CAUTH from {args.browser}. '
                f'{cookie_error or "No CAUTH cookie was found."} '
                'Make sure you are logged in on coursera.org in the selected browser.'
            )
        if source_browser != args.browser:
            logging.warning('Could not load CAUTH from %s; using the logged-in session from %s instead.',
                            args.browser, source_browser)
        logging.debug('Loaded CAUTH cookie from %s.', source_browser)
