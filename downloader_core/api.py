    ...
    def __call__(self, quiz_or_exam_json):
        ...
        for question_index, question_json in enumerate(quiz_or_exam_json['questions']):
            ...
            if question_type not in self.KNOWN_QUESTION_TYPES:
                ...
                logging.info('Capture the class name, quiz name, and data '
                             'above for local debugging')
...
class VideosV1(object):
    ...
    @staticmethod
    def _resolution_sort_key(video):
        match = re.search(r'(\d+)', str(video.resolution))
    ...
    def from_json(data):
        ...
        videos.sort(key=VideosV1._resolution_sort_key, reverse=True)
...
class DownloadTarget(object):
    class_name = attr.ib()
    path = attr.ib()
    specialization_name = attr.ib(default=None)
...
def resolve_download_targets(session, class_names, base_path='', specialization_hint=False):
    result = []
    seen = set()

    for class_name in class_names:
        parsed_target = general.parse_coursera_target(class_name)
        if parsed_target is None:
            continue

        should_expand_specialization = (
            parsed_target.kind == 'specialization' or
            (specialization_hint and not parsed_target.is_url)
        )

        if should_expand_specialization:
            specialization = SpecializationV1.create(session, parsed_target.slug)
            if specialization is not None:
                specialization_root = os.path.join(base_path, parsed_target.slug)
                for child_slug in specialization.children:
                    identity = (specialization_root, child_slug)
                    if identity in seen:
                        continue
                    seen.add(identity)
                    result.append(DownloadTarget(
                        class_name=child_slug,
                        path=specialization_root,
                        specialization_name=parsed_target.slug
                    ))

                logging.info('Expanded specialization "%s" into the following classes: %s',
                             parsed_target.slug,
                             ' '.join(specialization.children))
                continue

        identity = (base_path, parsed_target.slug)
        if identity in seen:
            continue
        seen.add(identity)
        result.append(DownloadTarget(
            class_name=parsed_target.slug,
            path=base_path,
...
class CourseraOnDemand(object):
    ...
    def _get_notebook_folder(self, url, jupyterId, **kwargs):
        ...
        for content in reply['content']:
            ...
            if content['type'] == 'directory':
                ...
                # way to do it."
                # Original upstream compatibility note preserved here.
    ...
    def extract_links_from_lecture(self, course_id,
                                   ...
                                   resolution=general.DEFAULT_VIDEO_RESOLUTION):
        """
        ...
        for element in dom['elements']:
            ...
            #
                ...
                logging.warning(
                    ...
                    'files, keep this output for local debugging.',
    ...
    def _extract_videos_and_subtitles_from_lecture(self,
        ...
        normalized_resolution = (resolution or general.DEFAULT_VIDEO_RESOLUTION).strip().lower()

        if normalized_resolution in ('best', 'max', 'highest', 'highest-available'):
            source = videos.get_best()
            logging.debug('Proceeding with highest available resolution %s of <%s>.',
                          source.resolution, video_id)
        elif resolution in videos:
